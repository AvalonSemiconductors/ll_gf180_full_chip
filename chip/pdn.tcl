...
# stdcell power pins
add_global_connection -net {vddcore} -pin_pattern {^VDD$} -power
add_global_connection -net {vsscore} -pin_pattern {^VSS$} -ground

# padframe core power pins
add_global_connection -net {vddcore} -pin_pattern {^DVDD$} -power
add_global_connection -net {vsscore} -pin_pattern {^DVSS$} -ground

global_connect

# core voltage domain
set_voltage_domain -name {CORE} -power {vddcore} -ground {vsscore}
...
add_pdn_connect -grid stdcell_grid -layers "$::env(FP_PDN_VERTICAL_LAYER) Metal3"
